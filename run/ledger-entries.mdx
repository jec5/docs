...
When a ledger entry expires, all of its available (unconsumed) balance is zeroed out and recorded in the _Currency Expired_ column. A new, negative ledger entry of type "Expiration" is created to deduct the expired amount and balance the now-expired positive ledger.
